...
def finder(name):
    replace = [".avi","1.4","5.1","-","_","DVDRip","BRRip","XviD","1CDRip","aXXo","[","]","(",")","{","}","{{","}}"
    ...
    "Exclusive","171","DiDee","v2","YTS","AG","UK","PIRATE"
    ...
    key="6c909847"
    if year!=0:
        url = "http://www.omdbapi.com/?t="+name+"&y="+str(year)+"&apikey="+key
        response = urllib.urlopen(url).read()
        jsonvalues = json.loads(response)
        print(str(jsonvalues))
        if jsonvalues["Response"]=="True":
            movieurl = "http://www.imdb.com/title/" + jsonvalues['imdbID']
            webbrowser.open_new_tab(movieurl)

    else:
        url = "http://www.omdbapi.com/?t="+name+"&apikey="+key
        response = urllib.urlopen(url).read()
        jsonvalues = json.loads(response)
        if jsonvalues["Response"]=="True":
            movieurl = "http://www.imdb.com/title/" + jsonvalues['imdbID']
